...
Gizmo Snap = Aligns translation to the visual work grid (absolute or relative mode) and quantizes rotation to the configured increment<br>
Snap Target = Switches translation snapping between the grid and nearby item anchors/mesh vertices<br>
Temporary Snap = Hold Ctrl while dragging to temporarily invert the Enable Snap setting<br>
Snap to Floor = Context-menu command that drops selected items onto terrain or the highest item surface below them<br>
Work Grid = Uses the XY top plane; the infinite grid follows the camera while keeping a stable world origin<br>
Grid Link = Keeps the visual grid size and translation snap increment synchronized<br>
